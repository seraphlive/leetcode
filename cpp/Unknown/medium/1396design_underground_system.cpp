...
public:
    ...
    void checkOut(int id, string stationName, int t)
    {
        ...
        auto& [sumDuration, cnt] = stats[startStation + ">" + stationName];
        sumDuration += t - timeIn;
        ++cnt;
    }
    
    double getAverageTime(string startStation, string endStation)
    {
        auto [sumDuration, cnt] = stats[startStation + ">" + endStation];
        return static_cast<double>(sumDuration) / cnt;
    }
private:
